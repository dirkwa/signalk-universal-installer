...
**There is no `CHANGELOG.md` and there should not be one.** The tag is the release act and the notes are generated from the PRs merged since the previous tag, so a hand-maintained file would only ever disagree with them. `.github/release.yml` shapes that output: it drops dependabot and `skip-changelog` PRs and groups the rest under Features / Fixes / Documentation / Dependencies, with a wildcard `Other changes` catching everything unlabelled. GitHub categorises by **label**, not by commit type — it cannot read the Angular type out of a title — so an unlabelled PR lands in `Other changes` rather than vanishing. Labelling is optional refinement, not a requirement for a PR to appear.
