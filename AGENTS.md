...
| `~/.config/systemd/user/app.slice.d/50-signalk-cpu-priority.conf` | installer (`install.sh` step 4e), removed by `scripts/uninstall.sh` | `[Slice] CPUWeight=300` on the user manager's `app.slice`, where the Quadlet units run, so the SK stack outranks the containers signalk-container starts in `user.slice` 3:1 under CPU contention. A weight, never a cap. `daemon-reload` applies it live; deletion does not restore 100 until re-login, so uninstall also runs `set-property --runtime app.slice CPUWeight=100`. `scripts/doctor.sh` reads the live `cpu.weight`. |
