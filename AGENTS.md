...
**release-please writes the tag; don't hand-cut one.** `.github/workflows/release-please.yml` maintains a standing Release PR titled `chore(release): X.Y.Z` — the same Angular form every other PR uses, and the form `.coderabbit.yaml` skips — built from the conventional commits on master. Merging that PR is the release act: it creates the tag and the GitHub Release. The version comes from the commit types — a `feat` of any scope drives a minor — so steer an off-policy bump with an empty commit carrying a `Release-As: X.Y.Z` footer rather than editing `version.txt` (which release-please owns, along with `.release-please-manifest.json`).

Two things about that workflow break the release quietly if changed. **Removing the dispatch step stops the tag reaching the other workflows**: a tag pushed with `GITHUB_TOKEN` does not trigger other workflows (GitHub's recursion guard), and it fails silently — so release-please explicitly dispatches `release-branch.yml` and `pages.yml` after tagging. Remove that step and the Release still looks correct while the `release` branch stops advancing and Pages keeps serving the previous version. Add a tag-triggered workflow and it needs a dispatch line too; read the triggers rather than grepping for `tags:`, since a `push:` with no filter fires on tags as well. **The `gate` job reads each commit's subject line**, which holds only because this repo squash-merges. Enabling merge commits would put the Angular title on a later line, every push would judge non-releasable — the release PR's own merge included — and releases would stop.
...
| `~/.config/systemd/user/app.slice.d/50-signalk-cpu-priority.conf` | installer (`install.sh` step 11a-bis), removed by `scripts/uninstall.sh` and `signalk uninstall` | `[Slice] CPUWeight=300` on the user manager's `app.slice`, where the Quadlet units run, so the SK stack outranks the containers signalk-container starts in `user.slice` 3:1 under CPU contention. A weight, never a cap. `daemon-reload` applies it live; deletion does not restore 100 until re-login, so uninstall also runs `set-property --runtime app.slice CPUWeight=100`. `scripts/doctor.sh` reads the live `cpu.weight`. |
