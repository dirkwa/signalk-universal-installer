...
| `~/.signalk-updater/bug-reports/` (Windows only) | Windows `signalk` shim | Staging dir for `signalk bug-report` on Windows. The shim runs the in-VM bundler with `--to` here (on-disk, not tmpfs), then copies the tarball out to the Windows Desktop and deletes the VM-side copy. Transient — not created on Linux/macOS, where `bug-report` writes straight to a host-reachable path. |
