...
There is no npm publish for this repo — it's pure bash. GitHub Pages publishes the script tree on every push to `master` and on `v*` tags. Tag triggers add the tag name as `INSTALLER_VERSION` via a `sed` substitution at deploy time. A version tag (`vX.Y.Z` — deliberately narrower than the Pages `v*` trigger, which also matches non-version v-tags like `v1-keeper-final`) also force-updates the `release` branch (`.github/workflows/release-branch.yml`) — the channel `signalk devpod up` clones — so master is free to break between tags without breaking new dev workspaces.
...
| `~/.signalk-updater/` | updater container + installer | Tokens, `hardware.json` (detected serial / CAN / Bluetooth / GPIO, plus an optional `socketcanCandidate` written by `signalk socketcan`), logs. |
...
| `~/.signalk-doctor/installer-payload/` | doctor container (writer) + `signalk-bluetooth` helper (reader) | Quadlet templates + detect-hardware.sh staged verbatim by the doctor's `/api/installer/refresh` (`signalk update`). Inert until consumed: the bash installer's render step reads them, and `signalk bluetooth enable` installs `signalk-dbus-proxy.container.template` from here (substituting the pinned proxy image) when the live quadlet is missing. |
