...
**release-please writes the tag; don't hand-cut one.** `.github/workflows/release-please.yml` maintains a standing Release PR titled `chore: release vX.Y.Z`, built from the conventional commits on master. Merging that PR is the release act: it creates the tag and the GitHub Release. The version comes from the commit types — a `feat` of any scope drives a minor — so steer an off-policy bump with an empty commit carrying a `Release-As: X.Y.Z` footer rather than editing `version.txt` (which release-please owns, along with `.release-please-manifest.json`).
