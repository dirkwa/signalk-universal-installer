...
1. Detects host (OS, arch, distro family) and runs pre-flight: RAM ≥ 2 GB, disk ≥ 5 GB (checked on `$HOME` and, when a `rootless_storage_path` or `XDG_DATA_HOME` puts the container store on a different filesystem, on that one too — the images land there), the signalk-server ports (80 + 443, or 3000 + 3443 if you decline standard ports) and 3003 / 3004 free, cgroups v2 with memory + pids delegation to the user slice (autofixed when possible), Podman ≥ 5.3.

   Preflight also checks free space in the directory `podman pull` stages into. Podman decompresses each image layer there before committing it to the store, so the directory has to hold the largest working set at once. The requirement is **768 MB**, overridable via `STAGING_REQUIRED_MB`. That figure comes from measuring the pull into a clean store: `signalk-server` peaked at 436 MB and `signalk-doctor-server` at 96 MB (2026-09-13). These images are rolling tags, so treat the measurements as the rationale for the threshold at the time it was set, not as current image facts — re-measure before changing it. This check **blocks** the install rather than warning: the pull cannot succeed without the space.

   This bites on hosts where that directory is RAM-backed. Podman stages in `TMPDIR` if it is set, otherwise `engine.image_copy_tmp_dir` (default `/var/tmp`) — so on a box where either lands on a tmpfs, the tmpfs size cap, not the disk, is the ceiling. Note that the default is `/var/tmp`, not `/tmp`, and some images mount it on tmpfs — the case this was reported from was a 4 GB CM4 whose `/var/tmp` was tmpfs at 256 MB, which cannot hold `signalk-server`'s staging peak at any point. The installer avoids this for its own pulls by pointing `TMPDIR`, for the duration of the pull only, at a `containers/tmp` directory beside the container store — it reads podman's effective `GraphRoot`, so a store moved via `rootless_storage_path` or `XDG_DATA_HOME` is followed rather than assuming `~/.local/share`. Staging beside the store means a store with room for the image has room to unpack it — the 5 GB disk check above covers the committed images on that same filesystem, and this 768 MB is the transient space a pull needs on top of it. Set `SK_STAGING_DIR` in the environment to override the location. The installer's own pulls are therefore covered regardless of what `TMPDIR` was set to in your shell — the per-command assignment wins. Preflight still prints the fix below, for the case its check reports: the container store's own filesystem is short on space, which the redirect cannot help with, or you run `podman pull` by hand outside the installer. It redirects podman permanently:
...
13. Installs the `signalk` command at `~/.local/bin/signalk` — a dispatcher for `health`, `recover`, `socketcan`, `bluetooth`, `timesync`, `update`, `hardware`, `render-server`, `resolv-watch`, `netgate-watch`, `resetadmin`, `bug-report`, `stop`, `start`, `restart`, `devpod`, and `uninstall`. Run `signalk help` for usage. (`signalk update` calls the doctor's `/api/installer/refresh` endpoint to rewrite the host-resident scripts and Quadlet templates. The refresh itself restarts nothing, but `update` then re-renders the live server Quadlet from the refreshed template, which restarts `signalk-server` if that changed anything — pass `--no-render` to skip it.) The installer also persists `~/.local/bin` on your PATH if it isn't already. Right after it, writes `~/.config/systemd/user/app.slice.d/50-signalk-cpu-priority.conf` (`[Slice] CPUWeight=300`, no sudo): the Quadlet units run in `app.slice`, the containers signalk-container starts through the podman socket run in `user.slice` next to it, and cgroup weights compare siblings — at the default 100:100 a chart import saturating every core takes half the CPU from signalk-server; 300:100 keeps the SK stack at three quarters under contention and changes nothing on an idle host. Ranking the plugin containers and jobs among themselves is signalk-container's own CPU-priority setting; `signalk uninstall` removes the drop-in again.
    ...
    Each pull logs the digest of the platform image it landed on — podman selects the host's architecture out of a multi-arch tag, so this is not the manifest-list digest the registry shows for that tag — followed by the image's `org.opencontainers.image.version` and the short form of its `org.opencontainers.image.revision` when the image carries those labels. Rolling tags are re-pointed every few hours, so the tag alone does not identify a build after the fact; the digest and the revision do, and the revision is the source commit the build came from. Both labels are baked in at build time, so they survive a pull onto an empty store, which makes the build pulled during that run identifiable from the log alone with no network. This is evidence about the pull, not about what is running now — the Versions tab can switch `signalk-server` to another image afterwards, and the log is truncated on the next installer run.
...
On Windows the "host" is the Podman machine VM, and a fresh one boots already reporting `80` with nothing under `/etc/sysctl.d` holding it there. The installer writes the drop-in regardless, because that transient value does not survive `wsl --shutdown`: without the file the floor returns to `1024` on the next start and signalk-server crash-loops on `listen EACCES: permission denied 0.0.0.0:80` while its container still reports `Up` — a stack that looks healthy and serves nothing on `:80`. On an install from before this was fixed, neither `signalk update` nor `signalk render-server` repairs it — the doctor's `/api/installer/refresh` rewrites host scripts and Quadlet payloads only, and touches nothing under `/etc/sysctl.d`. Re-run the installer, or set it by hand:
...
Result: the same container stack as Linux. With mirrored networking the Podman Machine VM shares the Windows host's LAN IP, so you reach the stack from **any device** at `http://<windows-host-ip>` — admin UI (`:3000` if you decline standard ports), `:3003` (Updater), `:3004` (Doctor). The installer prints the exact address at the end. **From the Windows box itself, use `http://127.0.0.1` instead.** Two separate quirks make the other two spellings worse there, and neither affects other devices:

- The **LAN IP does not answer from the Windows host** under mirrored networking, even though every other device on the network reaches it fine (verified: a phone loads `http://<host-ip>` while the same request from Windows is refused at the TCP layer, with the firewall rules present and the VM answering on that address internally). Mirrored mode gives the VM the host's address; the host reaching *itself* through it is the case that does not work.
- **`localhost` answers but is slow.** Windows resolves it to IPv6 (`::1`) first while the stack listens on IPv4, so every new connection stalls ~21 s on the IPv6 timeout before falling back — measured as 21.1 s over `localhost` against 0.01 s over `127.0.0.1`, same server, same moment.

`127.0.0.1` has neither problem. Other devices should use the LAN IP the installer prints.
...
On Windows, substitute the host's LAN IP (or `127.0.0.1`) for `localhost` in those two URLs — `localhost` works but waits out the IPv6 timeout first, which is the last thing you want when something is already broken.
