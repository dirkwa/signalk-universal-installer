...
1. Detects host (OS, arch, distro family) and runs pre-flight: RAM ≥ 2 GB, disk ≥ 5 GB, the signalk-server ports (80 + 443, or 3000 + 3443 if you decline standard ports) and 3003 / 3004 free, cgroups v2 with memory + pids delegation to the user slice (autofixed when possible), Podman ≥ 5.3.
   ...
   Preflight also checks free space in the directory `podman pull` stages into. Podman decompresses each image layer there before committing it to the store, so the directory has to hold the largest working set at once. The requirement is **768 MB**, overridable via `STAGING_REQUIRED_MB`. That figure comes from measuring the pull into a clean store: `signalk-server` peaked at 436 MB and `signalk-doctor-server` at 96 MB (2026-09-14). These images are rolling tags, so treat the measurements as the rationale for the threshold at the time it was set, not as current image facts — re-measure before changing it. This check **blocks** the install rather than warning: the pull cannot succeed without the space.

   This bites on hosts where that directory is RAM-backed. Podman stages in `TMPDIR` if it is set, otherwise `engine.image_copy_tmp_dir` (default `/var/tmp`) — so on a box where either lands on a tmpfs, the tmpfs size cap, not the disk, is the ceiling. A 4 GB CM4 with a 512 MB RAM-backed `/tmp` cannot pull `signalk-server` at all. The installer avoids this for its own pulls by pointing `TMPDIR` at `~/.local/share/containers/tmp`, on the same filesystem as the container store, for the duration of the pull only. The installer's own pulls are therefore covered regardless of what `TMPDIR` was set to in your shell — the per-command assignment wins. Preflight still prints the fix below, for the case its check reports: the container store's own filesystem is short on space, which the redirect cannot help with, or you run `podman pull` by hand outside the installer. It redirects podman permanently:
