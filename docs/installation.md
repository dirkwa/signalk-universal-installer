...
3. Enables `loginctl enable-linger $USER` so the stack survives reboots, and writes `~/.config/systemd/user/app.slice.d/50-signalk-cpu-priority.conf` (`CPUWeight=300`, no sudo): the Quadlet units run in `app.slice`, the containers signalk-container starts through the podman socket run in `user.slice` next to it, and cgroup weights compare siblings — at the default 100:100 a chart import saturating every core takes half the CPU from signalk-server; 300:100 keeps the SK stack at three quarters under contention and changes nothing on an idle host. Ranking the plugin containers and jobs among themselves is signalk-container's own CPU-priority setting.
