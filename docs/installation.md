...
- **Boot Scheduled Task.** A Podman machine does not start when Windows boots. The installer registers a task (`SignalK Podman Machine (signalk)`) that runs `podman machine start signalk`. To start with **nobody signed in**, the task must run "whether the user is logged on or not", which needs Windows to store this account's password — so the installer **prompts for it**. If you supply it, the task runs **at boot** (no sign-in needed). If you press Enter/Cancel to skip, the installer instead registers a **sign-in task** (no password stored) that starts the stack when you log in — so it's still automatic, just not before login. It can't be a Windows *service*: the machine is a per-user WSL2 distro and WSL2 only starts from a user session, which `LocalSystem`/session-0 services don't have — but a startup task running *as your user* does, even with no interactive sign-in. systemd inside the VM then starts the containers.

  > **Microsoft-account caveat.** Boot-without-login needs a Windows account whose password can be stored in the task — i.e. a **local** account. If your Windows account is **linked to a Microsoft account** (the common default), Windows refuses to store the credential (`Register-ScheduledTask` fails with `0x8007052E`), and the installer detects this and falls back to the sign-in task. For true boot-without-login on a boat PC, either **create a dedicated local Windows account and run the installer signed in as it** (the stack then lives under that account and the boot task works), or **enable Windows auto-login** for your account (e.g. Sysinternals **Autologon**) so the sign-in task fires unattended at boot.
...
After a reboot (with no sign-in if you stored the password, otherwise after you log in), give the machine ~30–60 s, then the stack is reachable at `http://<windows-host-ip>` from any device.
